...
class _Steps3State extends State<Steps3> {
  ...
  // List<List<ExpansionTileController>> _controller = [];
  ...
  // void _resetDisplay() {
  //   while (_controller.length < _steps.length) {
  //     _controller.add([ExpansionTileController()]);
  //   }
  //   for (var i = 0; i < _steps.length; i++) {
  //     while (_controller[i].length < _steps[i].length) {
  //       _controller[i].add(ExpansionTileController());
  //     }
  //   }

  //   for (var i = 0; i < _controller.length; i++){
  //     for (var j = 0; j < _controller[i].length; j++){
  //       _controller[i][j].collapse();
  //     }
  //   }
  // }
    ...
    return Container(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: ExpansionTile(
            // controller: _controller[superIndex][index],
            title: Column(children: content[0]),
            children: content[1],
          )
        ),
      ),
  ...
  Widget _genStep(int index, List<List<dynamic>> content) {
    ...
    if (content.length > 1) {
      ...
      w_show = ExpansionTile(
        title: Column(children: List<Widget>.from(content[0])),
        children: w,
      );
    }
    else{
      w_show = Column(children: List<Widget>.from(content[0]));
    }
